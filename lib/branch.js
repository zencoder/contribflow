...
branch.current = function(options){
  ...
  info = branch.parseName(stdout.replace('\n', ''));
...
branch.update = function(name, options){
  ...
  log('Updating the '+name+' branch with '+options.remote+' changes');
  return shell.exec('git pull '+options.remote+' '+name);
};

branch.checkout = function(name){
  log('Checking out the '+name+' branch');
...
branch.deleteLocal = function(name){
...
branch.pushTags = function(options){
  options = options || {};
  var remote = options.remote || 'origin';
